...
1. 生成id按时间趋势递增
2. 整个分布式系统不会产生重复id(因为有data center id和worker id 做区分

优点

1. 毫秒数在高位自增数在低位，id都是趋势递增
2. 不依赖第三方库，生成id性能高，非常灵活
3. 信息安全
4. 含时间戳

缺点

1. 依赖机器时钟，如果时钟回拨将可能导致重复id生成。

2. 分布式情况下，由于不是每台机器时钟都是同步的，所以有时候会出现不是全局递增的情况。

   ps:传统的雪花算法实现都是通过System.currentTimeMillis()来获取时间并与上一次时间进行比较，这样的实现严重依赖服务器的时间。百度开源**UidGenerator**的时间类型是AtomicLong，且通过incrementAndGet()方法获取下一次的时间，从而脱离了对服务器时间的依赖，也就不会有时钟回拨的问题。
